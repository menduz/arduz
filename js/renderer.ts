import {TextureDB} from './renderer/texture-db';
import {Grh} from './renderer/grh';
import {KeyStates} from './input/key-states';
...
var HeadFactory = (function () {
	...
	return function (index) {
		return (DB[index] = DB[index] || [
			Grh(grhCabezas, 16, 16, 0, index * 16),
			Grh(grhCabezas, 16, 16, 16, index * 16),
			Grh(grhCabezas, 16, 16, 48, index * 16),
			Grh(grhCabezas, 16, 16, 32, index * 16)
		]);
	}
})();
	...
	var ArmarCuerpo = function (body, grh) {
		...
		for (var h = 0; h < arrays.length; h++) {
			for (var i = 0; i < arrays[h].length; i++) {
				arrays[h][i] = Grh(grh, AnchoCuerpos, AltoCuerpos, i * AnchoCuerpos, h * AltoCuerpos)
			};
		};
	...
	return function (body, head, alto) {
		...
		return {
			...
			render: function (elapsedTime, x, y) {
				...
				this.body[_heading][
					!enMovimiento ? 0 : (((new Date).getTime() / 60) | 0) % this.body[_heading].length
				](tmpx, tmpy);
...
var GrhFromCache = function (URL: HTMLImageElement | string | HTMLCanvasElement) {
	var t = null;
...
var Map = (function () {
	...
	var loadTileset = function (tileset, url) {
		...
		TextureDB.once(url + '_loaded', tilesetLoaded);

		var ct = 0;
		for (var y = 0; y < 16; y++) {
			for (var x = 0; x < 32; x++) {
				Tileset[tileset].push(Grh(url, TileSize, TileSize, x * TileSize, y * TileSize));
			};
		};
		...
		loadMap: function (data?) {
			for (var x = 0; x < mapSize; x++) {
				...
				for (var y = 0; y < mapSize; y++) {
					mapData[x][y] = Tileset[0][/*Math.random()*32|0*/x % 4 + (y % 4) * 32];
				}
			}
		}
	}
...
var UI = (function () {
	...
	var hotBar = Grh('cdn/ui/hotbar.png', 400, 64, 0, 0);
	var barras = Grh('cdn/ui/barras.png', 200, 64, 0, 0);
	var frameInterno = Grh('cdn/ui/frameinterno.png', 800, 600, 0, 0);
	...
	return {
		render: function () {
			...
			globalContext.drawImage(TextureDB.get('cdn/adz.png'), -100, -100, 640, 480);
		},
...
var populate = (function () {
	...
	chars.push(myChara); setInterval(function () { myChara.moveByHead(((Math.random() * 700) % 4) | 0) }, 192)
})