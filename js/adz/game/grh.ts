define(['js/adz/mzengine/textures', 'js/adz/mzengine/mzengine'], function(textures, engine){
	var indexaciones = {};
	var realizados = {};
	...
	this.indexarFrame = function(index, grafico, srcX, srcY, w, h){
		indexaciones[index] = {
			g: grafico, 
	...
	this.indexarAnimacion = function(index, frames, tiempo){
		indexaciones[index] = {
			g: null, 
	...
	this.cargarGraficosRaw = function(url, cb){
				...
				for(var i in datos){
					...
					if(d && d.length){
						if(t = grhHeader.exec(d)){
							that.indexarFrame(t[1], t[2], t[3], t[4], t[5], t[6]);
						} else {
							if(t = animHeader.exec(d)){
								that.indexarAnimacion(t[1], t[3].split(/-/g), parseInt(t[4]));
							}
						}
					}
				}
